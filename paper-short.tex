...
%Typically, such theories are developed in terms of specific mechanistic models of human language processing; for example, \citet{hawkins1994performance} shows that word order patterns can result from optimization for efficient parsing under a particular parsing algorithm.
...
% Nice phrases from Sims (2018):
% biological information processing should seek to maximize performance subject to constraints on information processing capacity.
% The current results can be concisely stated as follows:
% Rate-distortion theory provides analytical and numerical tools for solving such constrained optimization problems (12–14).
...
\end{equation}
which will on average be larger than Eq.~\ref{eq:surp} (see SI for proof). The limitations of memory have well-documented empirical effects on language processing, causing difficulty above and beyond the difficulty associated with raw information content \citep{gibson1998linguistic,gibson1999memory,gibson2000dependency,vasishth2005activationbased,levy2013memory}.
