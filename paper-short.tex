...
However, it's been hard to concluseively take this as evidence for efficient processing under resource limitations because tests have been tied to specific algorithmic models of language processing.\mhahn{TODO make this better}
\mhahn{Maybe there is a citation that explicitly mentions this as a limitation of  thigngs like DLM}
...
\mhahn{reader will have lost the plot by now. strogn promise in the beginning. can solve this by foreshadowing}
...
\includegraphics[width=0.2\textwidth]{figures-gdrive/tradeoff.png}
	\caption{
		...
		(b) Here we illustrate our theoretical result. We plot $I_t$ (top) and $tI_t$ (bottom) as functions of $t$. A listener using $B$ bits memory (bottom) to represent prior observations will incur at least $A$ bits of extra surprisal beyond the entropy rate (top). \mhahn{there is t and T. COnfusing}
