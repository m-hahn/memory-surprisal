...
assert model in ["REAL", "RANDOM_BY_TYPE", "GROUND"]
...
#      assert line["dep"] == "root"
   ...
   inner = {"word" : line["posUni"], "category" :line["posUni"], "children" : None, "line" : line}
   while rightChildren:
      sibling = rightChildren.pop(0)
      inner = {"category" : line["posUni"]+"_R_", "children" : [inner, sibling], "line" : line}
   while leftChildren:
      sibling = leftChildren.pop(-1)
      inner = {"category" : line["posUni"]+"_L_", "children" : [sibling, inner], "line" : line}
   return inner
...
if model == "RANDOM_MODEL":
  ...
  files = [x for x in os.listdir(groundPath) if x.startswith(language[:language.rfind("_")]+"_infer")]
  print(files)
  assert len(files) > 0
