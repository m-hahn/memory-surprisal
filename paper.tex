...
Following (CITE), we represented other words by their part-of-speech tags.
This affected languages with larger corpora (TODO).
...
Second, excluded words are extremely infrequent, occurring less than 10 times (except for Czech and Russian, the languages for which we have by far the largest datasets).
Many of the excluded words occur only once in the dataset.
This means that any model would only be able to extract very limited information on these words from the available data, likely \emph{less} than what is provided by the part-of-speech tag.
