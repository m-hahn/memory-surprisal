...
for language in languages:
   files = [x for x in os.listdir("/u/scr/mhahn/CODE/deps/") if x == "yCreateRandomModels_"+language+"_OnlyWordForms_BoundedVocab.py"]
   if len(files) == 0:
     ...
     with open(bestPriorModel, "r") as inFile:
         ...
         print("/u/scr/mhahn/CODE/deps/yCreateRandomModels_"+language+"_OnlyWordForms_BoundedVocab.py")
         with open("/u/scr/mhahn/CODE/deps/yCreateRandomModels_"+language+"_OnlyWordForms_BoundedVocab.py", "w") as outFile:
             ...
             print >> outFile, "   command = ['./python27', 'yWithMorphologySequentialStreamDropoutDev_BaselineLanguage_Fast_SaveLast_NoFinePOS_OnlyWordForms_BoundedVocab.py', language, language, dropout1, emb_dim, lstm_dim, layers, learning_rate, model, dropout2, batch_size,input_noising,  sequence_length]"
