...
memSurpPlot_constit_deps = function(language) {
    data = read.csv(paste(language,"_after_tuning_constit_deps.tsv", sep=""), sep="\t")
    library(tidyr)
    library(dplyr)
    library(ggplot2)
    ...
    ggsave(plot, file=paste(language,"-entropy-memory_constit_deps.pdf", sep=""))
    return(plot)
}

plot = memSurpPlot_constit_deps("PTB")




memListenerSurpPlot_constit_deps = function(language) {
    data = read.csv(paste(language,"_decay_after_tuning_constit_deps.tsv", sep=""), sep="\t")
    library(tidyr)
    library(dplyr)
    library(ggplot2)
    data = data %>% filter(Type %in% c("RANDOM_BY_TYPE", "REAL_REAL"))
...
data = rbind(data2, data)
    ...
    ggsave(plot, file=paste(language,"-listener-reverse-surprisal-memory_constit_deps.pdf", sep=""))
    ...
    ggsave(plot, file=paste(language,"-listener-reverse-surprisal-memory-by-run_constit_deps.pdf", sep=""))
    ...
    ggsave(plot, file=paste(language,"-listener-surprisal-memory-by-run_constit_deps.pdf", sep=""))
    ...
    ggsave(plot, file=paste(language,"-listener-surprisal-memory_constit_deps.pdf", sep=""))
...
plot = memListenerSurpPlot_constit_deps("PTB")
