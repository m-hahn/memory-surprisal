...
#    plot = plot + theme(legend.position="none")   
    plot = plot + geom_density(data= data%>%filter(Type == "RANDOM_BY_TYPE"), aes(y=..scaled.., weight=1/counts))      
    dataReal = data %>% filter(Type %in% c("REAL_REAL")) %>% group_by(Language, Type) %>% summarise(MI_z=mean(MI_z)) %>% mutate(y=1)
    plot = plot + geom_bar(data=dataReal,  aes(y=y, group=Type), width=barWidth, stat="identity", position = position_dodge()) 
    plot = plot + geom_text(data=dataReal, aes(y=1, label=Language), angle=90, hjust=-0.1)
    plot = plot + ylim(0, 1.5)
    ggsave(plot, file=paste("figures/full-REAL-listener-surprisal-memory-HIST_z_byMem_onlyWordForms_boundedVocab_labeled.pdf", sep=""))
