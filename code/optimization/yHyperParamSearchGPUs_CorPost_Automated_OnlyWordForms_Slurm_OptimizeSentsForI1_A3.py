...
bounds.append(['--lr_policy', float, 0.000001, 0.000002,0.000005, 0.00001, 0.00002, 0.00003, 0.00005, 0.0001, 0.0002, 0.001, 0.005, 0.01, 0.05, 0.1, 0.2, 0.5, 1.0]) #default=0.001, dest="lr_policy")
...
bounds.append(['--batchSize', int, 1,2,4,8, 16, 32])
...
version = "optimizeOrdersForI1_A3.py"
...
#   return theirXPs[i][0]
   if runningProcesses[i].poll() is not None:
      with open(DIRPATH+"/estimates-"+language+"_"+version+"_model_"+str(theirIDs[i])+"_"+"REINFORCE"+".txt", "r") as inFile:
         next(inFile)
