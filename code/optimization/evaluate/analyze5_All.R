...
data_ground_ = read.csv(paste("~/scr/CODE/memory-surprisal/results/manual_output_ground_coarse/", result, sep=""), sep="\t")
...
for(file in list.files(PATH)) {
   if(grepl(paste("#", language), paste("#", file))) {
     data = read.csv(paste(PATH, "/", file, sep=""), sep="\t") %>% rename(CoarseDependency=CoarseDependency) %>% mutate(Language=language)
     data_b = merge(data, data_ground, by=c("CoarseDependency"))
     pValue = cor.test(data_b$DistanceWeight, data_b$Distance_Mean_NoPunct)$p.value
     cat(file, " ", pValue, "\n")
     data$Model = file
