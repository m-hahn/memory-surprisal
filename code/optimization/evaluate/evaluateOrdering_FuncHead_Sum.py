...
parser.add_argument("--model1", dest="model1", type=str)
parser.add_argument("--model2", dest="model2", type=str)
...
assert "REAL" not in args.model1
assert "REAL" not in args.model2
   ...
   if model == "REAL" or model == "REAL_REAL":
     ...
     for key in range(len(itos_deps)):
        if itos_deps[key][1].split(":")[0] not in dhByDependency:
            dhByDependency[itos_deps[key][1].split(":")[0]] = 0
            distByDependency[itos_deps[key][1].split(":")[0]] = 0
            print("WARNING 290", key)
        dhWeights[key] = dhByDependency[itos_deps[key][1].split(":")[0]]
        distanceWeights[key] = distByDependency[itos_deps[key][1].split(":")[0]]
     originalCounter = "NA"
   return dhWeights, distanceWeights

dhWeights1, distanceWeights1 = read(args.model1)
dhWeights2, distanceWeights2 = read(args.model2)
...
for lamb in [x/3.0 for x in range(3)]:
   mistaken = 0
   correct = 0
   
   dhWeights = [lamb * x + (1-lamb) * y for x, y in zip(dhWeights1, dhWeights2)]
   distanceWeights = [lamb * x + (1-lamb) * y for x, y in zip(distanceWeights1, distanceWeights2)]
   ...
   results.append(correct/(mistaken+correct+0.0))
