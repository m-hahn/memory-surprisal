...
parser.add_argument('--dropout_rate_parser', type=float, dest="dropout_rate_parser", default=0.1)
...
parser.add_argument('--pos_embedding_size_parser', type=int, dest="pos_embedding_size_parser", default=50)
...
parser.add_argument('--input_dropoutRate_parser', type=float, dest="input_dropoutRate_parser", default=0.0)
...
components_lm = [word_embeddings_lm, rnn_lm, decoder_lm, baseline_lm, baseline_upos_lm, baseline_ppos_lm]
...
#           pos_p_layer = pos_p_embeddings_lm(pos_p_indices)
           input_concat = words_layer #torch.cat([words_layer, pos_u_layer, pos_p_layer], dim=2)
           inputEmbeddings = dropout_lm(input_concat) if train else input_concat

           output, hidden = rnn_lm(inputEmbeddings, None)
           baseline_predictions = log(vocab_size_lm) + baseline_lm(wordIndices) + baseline_upos_lm(pos_u_indices) + baseline_ppos_lm(pos_p_indices)
           ...
           for i in range(0,len(input_words)-1): 
              for j in range(args.batchSize):
                 if input_words[i+1][j] != 0:
                    policyGradientLoss_lm += (float(rewardMinusBaseline_lm[i][j]) * batchOrdered[j][-1]["relevant_logprob_sum"])
                    if input_words[i+1] > 2 and j == 0 and printHere:
                       print "\t".join([str(x) for x in [itos[input_words[i+1][j]-3], itos_pos_ptb[input_pos_p[i+1][j]-3], float(lossesWord_tensor[i][j].data.cpu().numpy()), float(baseline_predictions[i+1][j].data.cpu().numpy())]])
                    wordNum_lm += 1
       ...
       if counter % 50000 == 0:
          ...
          with open(save_path+"/manual_output_funchead_langmod_coarse_best_ud/"+args.language+"_"+__file__+"_model_"+str(myID)+".tsv", "w") as outFile:
             ...
             for i in range(len(itos_deps)):
                ...
                dependency = key
                print >> outFile, "\t".join(map(str,[counter, dhWeight, dependency, distanceWeight]))
