...
with open("best-langmod.csv", "r") as inFile:
...
with open("commands-lambda1-langmod.csv", "r") as inFile:
...
while len(languages) > 0:
   script = "optimizePredictability_OnlyWords_Symmetric.py"

   language = random.choice(languages)
   import os
   files = [x for x in os.listdir(relevantPath) if x.startswith(language+"_") and script in x]
   posCount = 0
   negCount = 0
   for name in files:
