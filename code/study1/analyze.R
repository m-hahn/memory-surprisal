...
data1 = read.csv("results-simulation-6-short.py.tsv", sep="\t") %>% mutate(Order = "Short")
data2 = read.csv("results-simulation-6-long.py.tsv", sep="\t") %>% mutate(Order = "Long")
data=rbind(data1, data2)



data = data %>% mutate(Distance_ = ifelse(Order == "Short" & Distance == 1.0, 1.3, Distance)) %>% filter(Distance<7)
...
plot = ggplot(data=data, aes(x=Distance_, y=MI, color=Order)) + 
	geom_step(data=data %>% filter(Order == "Short"), size=2, linetype=2) + 
	geom_step(data=data %>% filter(Order == "Long"), size=2, linetype=2) + 
...
data = data %>% mutate(Surprisal_ = Surprisal) #= ifelse(Order == "Short" & Memory < 2, 0.285, Surprisal))
data = data %>% mutate(Memory_ = Memory) #ifelse(Order == "Short" & Memory < 2, 1.79, Memory))


data_ = data %>% select(Surprisal_, Memory_, Order)

data_ = rbind(as.data.frame(data_), data.frame(Memory_=c(2.12, 2.35), Surprisal_=c(0.077, 0.077), Order=c("Short", "Short")))
