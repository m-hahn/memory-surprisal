  ...
  #      #       print("..........", j2, trainPref)
   ...
   #      print(k,countNgram)
    ...
    #          print(followingCount)
              ...
              probability = (max(countNgram - args.alpha, 0.0) + args.alpha * followingCount * lastProbability[j]) / countPrefix
   #           print(probability)
              newProbability[j] = probability
         else:
            newProbability[j] = lastProbability[j]
      elif k == 0:
              probability = (countNgram + args.delta) / (len(train) + args.delta * len(itos))
              newProbability[j] = probability
   lastProbability = newProbability 
   newProbability = [None for _ in idev]
   try:
       surprisal = - sum([log(x) for x in lastProbability])/len(lastProbability)
   except ValueError:
