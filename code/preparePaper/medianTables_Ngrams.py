...
#   line = languageKey[language]
   ...
   if not os.path.isfile("../../ngrams/figures/"+language+"-listener-surprisal-memory-MEDIANS_QUANTILES_onlyWordForms_boundedVocab.pdf"):
      continue
   components.append( "\includegraphics[width=0.25\\textwidth]{ngrams/figures/"+language+"-listener-surprisal-memory-MEDIANS_QUANTILES_onlyWordForms_boundedVocab.pdf}" )
...
for part in range(len(entries)/(COLUMNS*ROWS_PER_PART)+1):
  with open("../../medians_ngrams_"+str(part)+".tex", "w") as outFile:
