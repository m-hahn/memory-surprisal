...
#   line = languageKey[language]
   ...
   components.append( "\includegraphics[width=0.25\\textwidth]{neural/figures/"+language+"-listener-surprisal-memory-QUANTILES_onlyWordForms_boundedVocab_REAL.pdf}" )
...
for part in range(len(entries)/(COLUMNS*ROWS_PER_PART)+1):
  with open("../../quantiles_REAL_"+str(part)+".tex", "w") as outFile:
