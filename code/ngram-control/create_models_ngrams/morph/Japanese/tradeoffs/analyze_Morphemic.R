...
data = data %>% filter(Script == "forWords_Japanese_RandomOrder_Normalized_FullData.py")
...
ggsave(plot, file=paste("figures/Japanese-suffixes-byMorphemes-it.pdf", sep=""), height=4, width=4)
...
plot = plot + theme(axis.line = element_line(colour = "black"),
            panel.grid.major = element_blank(),
                ...
                panel.border = element_blank(),
                    panel.background = element_blank())
ggsave(plot, file=paste("figures/Japanese-suffixes-byMorphemes-memsurp.pdf", sep=""), height=4, width=4)
...
plot = plot + theme(axis.line = element_line(colour = "black"),
            panel.grid.major = element_blank(),
                ...
                panel.border = element_blank(),
                    panel.background = element_blank())
ggsave(plot, file=paste("figures/Japanese-suffixes-byMorphemes-auc.pdf", sep=""), height=4, width=4)
...
ggsave(plot, file=paste("figures/Japanese-suffixes-byMorphemes-auc-hist.pdf", sep=""), height=4, width=4)
...
plot = plot + theme(axis.line = element_line(colour = "black"),
            panel.grid.major = element_blank(),
                ...
                panel.border = element_blank(),
                    panel.background = element_blank())
ggsave(plot, file=paste("figures/Japanese-suffixes-byMorphemes-auc-heldout.pdf", sep=""), height=4, width=4)
...
plot = plot + geom_density(data= data%>%filter(Type == "RANDOM"), aes(y=..scaled..)) 
plot = plot + geom_bar(data = data %>% filter(!(Type %in% c("RANDOM"))) %>% group_by(Type) %>% summarise(AUC=mean(AUC)) %>% mutate(y=1),  aes(y=y, group=Type), width=barWidth, stat="identity", position = position_dodge())
ggsave(plot, file=paste("figures/Japanese-suffixes-byMorphemes-auc-hist-heldout.pdf", sep=""), height=4, width=4)
