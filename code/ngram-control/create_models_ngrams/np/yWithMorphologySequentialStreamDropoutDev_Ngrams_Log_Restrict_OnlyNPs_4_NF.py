...
def orderSentence(sentence, dhLogits, printThings):
   ...
   for line in sentence:
      if line["posUni"] == "NOUN":
         ...
         leftDependencies = [x["dep"] for x in childrenRight]
         if len(leftDependencies) == 0:
            continue
         if set(leftDependencies).issubset(set(["det", "nummod", "amod"])):
            ...
            if max(leftLengths+[0]) == 0:
              ...
              if "amod" not in leftDependencies or "det" not in leftDependencies:
                 continue
              dependents = [sentence[i-1] for i in line.get("children_HD", [])]
              print([x["dep"] for x in dependents])
              if model_[1] != "":
              ...
              nounPhrases.append([line] + dependents)
              if random() > 0.98:
...
devSurprisalTable = []
devSurprisalTables = {"amod" : ([]), "det" : ([]), "nummod" : ([]), "NOUN" : ([]), "EOS" : ([])}
for k in range(0,args.cutoff):
   ...
   surprisalByPOS = {"amod" : ([0,0]), "det" : ([0,0]), "nummod" : ([0,0]), "NOUN" : ([0,0]), "EOS" : ([0,0])}
...
#              assert abs(probability) > 1e-5, devW[idev[j]]
   for pos in surprisalByPOS:
     devSurprisalTables[pos].append(surprisalByPOS[pos][0] / (surprisalByPOS[pos][1]+1e-10))
