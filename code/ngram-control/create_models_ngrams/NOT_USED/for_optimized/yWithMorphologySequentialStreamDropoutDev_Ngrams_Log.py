...
parser.add_argument("--BASE_DIR", default="manual_output_funchead_langmod_coarse_best_balanced", type=str)
...
def initializeOrderTable():
   ...
   for partition in ["train", "dev"]:
     for sentence in CorpusIteratorFuncHead(args.language,partition, storeMorph=True).iterator():
...
if args.model == "REAL" or args.model == "REAL_REAL":
  ...
  originalCounter = "NA"
   ...
   inpModels_path = "/juicier/scr120/scr/mhahn/deps/"+args.BASE_DIR+"/"
   models = os.listdir(inpModels_path)
   models = filter(lambda x:"_"+args.model+".tsv" in x, models)
   if len(models) == 0:
...
#      head = line[header.index("Head")]
 #     dependent = line[header.index("Dependent")]
      ...
      key = dependency
      dhByType[key] = float(line[header.index("DH_Weight")])
      distByType[key] = float(line[header.index("DistanceWeight")])
...
corpusDev = CorpusIteratorFuncHead(args.language,"dev", storeMorph=True).iterator(rejectShortSentences = False)
dev = list(createStreamContinuous(corpusDev))[::-1]


corpusTrain = CorpusIteratorFuncHead(args.language,"train", storeMorph=True).iterator(rejectShortSentences = False)
