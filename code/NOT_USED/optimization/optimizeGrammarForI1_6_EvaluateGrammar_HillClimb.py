...
#  with open("/u/scr/mhahn/deps/locality_optimized_i1/Chinese_optimizeGrammarForI1_3.py_model_675523898.tsv", "r") as inFile:
  with open("/u/scr/mhahn/deps/memory-need-ngrams-auc-optimized/"+args.model, "r") as inFile:

     print(next(inFile))
     print(next(inFile))
     print(next(inFile))
     for line in inFile:
         dependency, value = line.strip().split(" ")
         weights[dependency] = int(value)
  originalCounter = "NA"
print(weights)
...
#          print("\t".join([str(sentCount), "Sentences"]))
       ordered = orderSentence(sentence, dhLogits, printHere)
