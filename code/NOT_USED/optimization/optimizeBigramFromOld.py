    ...
    for partition in partitions:
       ...
       if counter % 50000 == 0:
          ...
          with open(TARGET_DIR+"/"+args.language+"_"+__file__+"_model_"+str(myID)+".tsv", "w") as outFile:
             print >> outFile, "\t".join(map(str,["DH_Weight","CoarseDependency","DistanceWeight"]))
             for i in range(len(itos_deps)):
                key = itos_deps[i]
                dhWeight = dhWeights[i].data.numpy()
                distanceWeight = distanceWeights[i].data.numpy()
                dependency = key
                print >> outFile, "\t".join(map(str,[dhWeight, dependency, distanceWeight]))
       ...
       if counter >= 2000000:
          print "Stop after "+str(2000000)+" iterations"
          quit()
