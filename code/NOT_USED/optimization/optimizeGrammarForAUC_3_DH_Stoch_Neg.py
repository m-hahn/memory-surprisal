...
for iteration in range(20000):
  ...
  mostCorrect, mostCorrectValue = 1000000000, None
  for newValue in [-1] + [2*x+1 for x in range(len(weights))]: # + [weights[coordinate]]:
     if (weights[coordinate] < weights["HEAD"]) != (newValue < weights["HEAD"]):
        continue
     if random() < 0.0 and abs(newValue - weights[coordinate]) > 5 and (not (len(fullAUCs) >= 2 and abs(fullAUCs[-1] - fullAUCs[-2]) < 1e-5)):
        continue
