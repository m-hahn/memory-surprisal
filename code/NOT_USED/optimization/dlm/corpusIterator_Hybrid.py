...
def reverse_content_head(sentence):
   for i in range(len(sentence)):
     sentence[i]["func_dep"] = sentence[i]["dep"]
     sentence[i]["func_head"] = sentence[i]["head"]
   CH_CONVERSION_ORDER = ["cc", "case", "cop", "mark"]
   # find paths that should be reverted
   for dep in CH_CONVERSION_ORDER:
      for i in range(len(sentence)):
         if sentence[i]["func_dep"] == dep or sentence[i]["func_dep"].startswith(dep+":"):
             head = sentence[i]["func_head"]-1
             grandp = sentence[head]["func_head"]-1
             assert head > -1
             
             # grandp -> head -> i
             # grandp -> i -> head
             sentence[i]["func_head"] = grandp+1
             sentence[head]["func_head"] = i+1

             sentence[i]["func_dep"] = sentence[head]["func_dep"]
             sentence[head]["func_dep"] = "lifted_"+dep
             assert sentence[i]["index"] == i+1
   return sentence

class CorpusIteratorHybrid():
...
class CorpusIteratorHybridFraction():
