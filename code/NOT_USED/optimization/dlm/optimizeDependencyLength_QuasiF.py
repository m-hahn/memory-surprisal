...
from corpusIterator_Hybrid import CorpusIteratorHybrid as CorpusIterator
...
def orderSentence(sentence, dhLogits, printThings):
   ...
   for line in sentence:
      ...
      if printThings: 
         print "\t".join(map(str,["ORD", line["index"], (line["word"]+"           ")[:10], ("".join(list(key)) + "         ")[:22], line["head"],line["func_head"], dhSampled, direction, (str(probability.data.numpy())+"      ")[:8], str(1/(1+exp(-dhLogits[key])))[:8], (str(distanceWeights[stoi_deps[key]].data.numpy())+"    ")[:8] , str(originalDistanceWeights[key])[:8]    ]  ))
   ...
   for i, x in enumerate(linearized):
      moved[x["index"]-1] = i
   for i,x in enumerate(linearized):
      if x["func_head"] == 0: # root
         x["reordered_head"] = 0
      else:
         x["reordered_head"] = 1+moved[x["func_head"]-1]
...
relevantPath = "/u/scr/mhahn/deps/locality_optimized_dlm/manual_output_funchead_coarse_depl_quasiF/"
...
while True:
  ...
  while True:
    ...
    for partition in partitions:
           ...
           for i in range(1,len(input_words)): 
              for j in range(1):
                 if input_words[i][j] != 0:
                    ...
                    if batchOrdered[j][i-1]["func_dep"] == "root":
                       continue
       ...
       if counter % 10000 == 0:
          TARGET_DIR = "/u/scr/mhahn/deps/locality_optimized_dlm/"
          print "Saving"
