...
#      assert line["dep"] == "root"
   ...
   head = line["word"]
   if random() < args.REPLACE_WORD_WITH_PLACEHOLDER:
      head = "#"
   if countsByPartition["train"].get(head, 0) < 1 or countsByPartition["dev"].get(head, 0) < args.VOCAB_FOR_RELATION_THRESHOLD:
      head = "#"
...
for preterminal in terminals:
  ...
  for word in list(terminals[preterminal]):
    if countsByPartition["train"].get(word,0) < args.OOV_THRESHOLD_TRAINING or countsByPartition["dev"].get(word,0) < 1:
