...
#       print(comparisonMeanPerREAL.size())
       ...
       #   print(comparisonMeanForThisREAL/randomCount, comparisonMeanForThisREAL, p, probMedian)
          totalPValue += probMedian * p
          totalMean += probMedian * comparisonMeanForThisREAL/randomCount

#          ci = scipy.stats.binom.interval(0.001, comparisonMeanForThisREAL, randomCount, 0.5)
          ci = (statsmodels.stats.proportion.proportion_confint(count=comparisonMeanForThisREAL,nobs=randomCount, alpha=0.001, method="jeffreys"))
          totalLower += ci[0]*probMedian
          totalUpper += ci[1]*probMedian

#       print(comparison)
       print "\t".join(map(str,[language, real, i, totalMean, totalPValue, totalLower, totalUpper]))
