...
#  print("Mem", cumulativeMemory[0])
 # print("MI", cumulativeMI[0])
  ...
  means = torch.mean(interpolated, dim=0)
  sd = torch.sqrt(torch.clamp(torch.mean(torch.pow(interpolated, 2), dim=0) - torch.pow(means, 2), min=0))
  se = sd / math.sqrt(interpolated.size()[0])

  # get 25%, 50%, 75% quantiles

  for i in range(se.size()[0]):
     print("\t".join(map(str, [language, i, typ, float(xPoints[i]), float(means[i]), float(sd[i]), float(se[i])])))
