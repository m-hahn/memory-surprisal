   ...
   for partition in ["train", "dev"]:
     for sentence in CorpusIterator(language,partition).iterator():
      for line in sentence:
          ...
          infostruc = line["infostruc"]   #             assert line[-1] in ["N", "t", "f", "c"] or line.endswith("UNK") or line.endswith("NULL"), line
...
def orderSentence(sentence, dhLogits, printThings, dhWeights, distanceWeights):
   ...
   for line in sentence:
      ...
      key = (line["coarse_dep"], line["infostruc"])
      ...
      if printThings: 
         print "\t".join(map(str,["ORD", line["index"], (line["word"]+"           ")[:10], ("_".join(list(key)) + "         ")[:22], line["head"], dhSampled, direction, (str(probability.data.numpy())+"      ")[:8], str(1/(1+exp(-dhLogits[key])))[:8], (str(distanceWeights[stoi_deps[key]].data.numpy())+"    ")    ]  ))
...
#       print [name, pyro.param(name).data.numpy()]
    ...
    if step % 2000 == 0:
       ...
       with open(save_path+"/manual_output_ground_coarse_infostruc/"+language+"_"+__file__+"_model_"+str(myID)+".tsv", "w") as outFile:
